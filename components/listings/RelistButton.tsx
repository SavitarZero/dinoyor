...
export function RelistButton({ listingId, disabled }: { listingId: string; disabled?: boolean }) {
  ...
  return (
    <>
      <button
        onClick={() => disabled ? setShowModal(true) : setShowModal(true)}
        className="w-full py-1.5 rounded border border-green-700/40 text-green-400 text-xs font-medium text-center hover:bg-green-900/20 transition-colors"
      >
        Relist
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-2xl border border-border bg-surface p-5 space-y-4">
            {disabled ? (
              <>
                <div>
                  <p className="text-white text-sm font-bold">Cannot relist</p>
                  <p className="text-yellow-400 text-xs mt-1">This listing has an active order that hasn't been completed yet. You can relist after the order is confirmed or cancelled.</p>
                </div>
                <button
